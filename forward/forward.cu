...
template <unsigned int block_size>
__global__
void forward_max(HMM hmm, ObsSeqs seqs, float *alpha1, float *alpha2, float neginf, float *result) {
  int64_t task_index = blockIdx.x;
  float *alpha;
  if (seqs.maxlen & 1) {
    alpha = alpha1;
...
void forward(
  ...
  forward_max<512><<<seqs.num_instances, 512>>>(hmm, seqs, alpha1, alpha2, neginf, result);
  forward_lse<512><<<seqs.num_instances, 512>>>(hmm, seqs, alpha1, alpha2, neginf, result);
}

// A merged version of the above kernel, where the IR compiler has merged the
// separate tasks into a single one. This can be performed when:
// 1. The tasks are launched in sequence after each other with no CPU code
//    running in-between.
// 2. All tasks are launched with the same number of parallel instances.
// 3. We can run each task instance on a single block.
//
// The main benefit of this approach is that we reduce the number of kernels
// that we launch. This allows the tasks to run independently, instead of
// having to synchronize (globally) between each kernel call, which drastically
// improves performance (for this model).
template <unsigned int block_size>
__global__
void forward_merged(
    HMM hmm, ObsSeqs seqs, float *alpha1, float *alpha2, float neginf,
    float *result) {
  {
    int64_t task_index = blockIdx.x;
    int64_t instance = task_index;
    uint8_t o = seqs.data[instance * seqs.maxlen];
    for (int64_t state = threadIdx.x; state < 1024; state += block_size) {
      alpha1[instance * 1024 + state] = hmm.initial_prob[state] + hmm.output_prob[o * 64 + state % 64];
    }
  }
  __syncthreads();
  {
...
void forward_merged(
  ...
  float neginf = -1.0 / 0.0;
  forward_merged<512><<<seqs.num_instances, 512>>>(hmm, seqs, alpha1, alpha2, neginf, result);
}
